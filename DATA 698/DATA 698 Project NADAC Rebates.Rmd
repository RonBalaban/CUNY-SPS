...
(Optional) Run this code with the environment file in chosen directory, in order to save time and not run functions/data manipulations/API calls
```{r}
# Save environment
...
################################################################################
# Save data for shiny app project
...
(Working) Scrape all tables, and combines them into tibble for a drug
```{r}
...
scrape_clincalc = function(drug){
  ...
  # Ensure each table has column names, assign them manually if missing
...
(Working) Get list of all drugs, call the above function, and store the data 
```{r}
...
```{r}
# Initialize empty tibble to hold drug data
...
all_brand_names <- all_drug_names %>%
  # Separate the Brand_Names by comma and create a row for each brand name
...
Check_Drug_Duplicates_Optimized <- function(all_drug_names) {
  # This functions looks at which drugs have overlapping brand/generic names, and calls them out. 
  ...
  # Summarize to list shared names
...
# See drugs with re-occurring names in other drugs
potential_duplicates <- Check_Drug_Duplicates_Optimized(all_drug_names)
```

This is useful for if I decide to aggregate drugs together, but I'll keep them separate for now- I want to preserve the granularity of each drug, even if there might be some overlap. For example, Fluticasone vs Fluticasone; Salmeterol or Metformin vs Metformin; Sitagliptin
...
Load in Product information all the different drugs and possible NDC's
```{r}
# Set file path
file_path_product <- "C:/Users/Ron/OneDrive/Desktop/Personal - Important/Grad School Stuff/CUNY SPS/DATA 698 Spring 2025/Resources/product.xlsx"  
...
(Working) Pull all NDC's for a drug by brand, generic, proprietary name V1
```{r}
Pull_NDC <- function(drug_name, all_drug_names, ndc_data) {
  # Pull all NDC's for a drug by brand, generic, proprietary name V1. 
  ...
  # Filter using combined exactish pattern
...
(Working) Pull all NDC's for a drug by brand, generic, proprietary name V2
```{r}
Get_NDCs_For_Drug <- function(drug_name, brand_data, generic_data, ndc_data) {
  # Pull all NDC's for a drug by brand, generic, proprietary name V2
...
This function is later redefined at the bottom for when specifying the brand and generic NDC's, but this version works as needed for now
...
The fuzzy match gives us this many NDC's for the previously empty drugs:
...
```{r}
# drugs where exact match found 0 NDCs but fuzzy match found some
...
# Removing 'Amlodipine; Benazepril' as R can't seem to parse all the NDC's regardless of how I format them.
...
(Working) Break out the NDC's for each drug
```{r}
...
This package information was found in the same location as the NDC Product data, and the purpose of joining to it is to see the following fields:
...
```{r}
# Set file path
file_path_package <- "C:/Users/Ron/OneDrive/Desktop/Personal - Important/Grad School Stuff/CUNY SPS/DATA 698 Spring 2025/Resources/package.xlsx"

# Load in data
package_data <- read_excel(file_path_package)

# See which NDC's appear multiple times
...
I want to create 2 new dataframes based off of drug_ndc_detail and package_data, depending on whether or not they join and get package info
```{r}
...
drug_ndc_detail has 40,597 rows, 193 of which don't have corresponding NDC's in package_data. Those that do match and appear in the package_data, when joined on drug_ndc_detail creates a one-to-many relationship for drugs. drug_ndc_detail_package has 93,415 rows, which occurs because each PRODUCTNDC has different NDCPACKAGECODES. For example, the NDC 0002-1433 for the drug Dulaglutide, has 2 different package codes: 
...
AWP (Average Wholesale Price)	    	; Price the pharmacy pays a wholesaler, but an often inflated benchmark.
...
This site allows me to search drugs based by NDC Description (Which is effectively Drug_Name and PACKAGEDESCRIPTION together), NDC, and it contains the old NADAC, new NADAC, Percent Change, Start Date, End Date, and Effective Date for these price changes. However, I noticed that the NDC's are formatted differently here. For example, when 'NDC Description' is 'SILDENAFIL 50 MG TABLET', the NDC is '65862069030'. 

However, in my dataframe drug_ndc_detail_package, I have NDC's in this format: 65862-691. I also have NDCPACKAGECODE in this format: 65862-691-01.
The NADAC website I provided has 11 digits NDC's, which is the standardized NDC format used by CMS (Centers for Medicare and Medicaid Services). As mentioned, this contains the Labeler code (5 digits), Product Code (4 digits), and Package Code (2 digits). NADAC helps CMS determine payment instead of using manufacturer-reported list prices such as WAC or AWP. Additionally, individual State Medicaid agencies use NADAC to determine payment for drug ingredient costs instead of calculating their own Average Acquisition Cost (AAC). More than 40 states utilize NADAC pricing to compute, in part, the ingredient costs of Medicaid-covered outpatient drugs. [https://www.analysource-public.com/drug-price-info/]
...
For example, I checked drug_ndc_detail_package for for all rows when Drug_Name = 'Sildenafil', and when 'NDC' = '13668-185', I get back 5 rows. One of those rows has NDCPACKAGECODE = '13668-185-90'. The NADAC website does in fact have results for when NDC = '13668018590'. Another example, when NDCPACKAGECODE = 13668-185-05, I see a corresponding NDC for 13668018505. It seems that these are actually identical, just formatted a bit differently.

For another example, I checked my dataframe for when NDC = 99207, and I have 2 rows. These rows have NDC = '99207-300', and NDCPACKAGECODE = '99207-300-30' and '99207-300-60'. I checked the website for when NDC starts with 99207 and contains 300, and I do Indeed see results for when NDC = '99207030030' or NDC = '99207030060'. This confirms what I saw in my first example above, that this website does have the data I need, in a different format.
...
(Working) Reformat base NDC to the standard 11-digit CMS NDC
...
As the NADAC dataset is too large and contains 2.7 million rows, I'll need API functionality to call just the NDC's I need
```{r}
library(httr)
library(jsonlite)
library(dplyr)
...
I'm looking for the dataset that has these fields:
    ...
    old_nadac_per_unit
    new_nadac_per_unit
    classification_for_rate_setting (un-needed)
    percent_change (un-needed)
    primary_reason (un-needed)
    start_date (un-needed)
    end_date (un-needed)
    effective_date
...
(Working) Query for first N rows
```{r}
...
(Working) Query all data for all rows for specific NDC.
```{r}
library(httr)
library(jsonlite)
...
query <- paste0(
  ...
  "[WHERE ndc = \"", ndc_val, "\"]"
  #"[LIMIT 10]"  #If I only want specific number of rows
)
...
(Working) Query all data for all rows for specific NDC, annual-level averages
```{r}
library(httr)
library(jsonlite)
library(dplyr)
...
The record on 9/19/2018 is far too high compared to all the other observations for this field in the entire data for NDC 65862069030, which ranges normally from 0.08516 to 0.66538. Any outlier values that are  much larger than all other values (for either the old NADAC per unit or new NADAC per unit) can throw off the estimates for future forecasting. That high value could represent a real, albeit temporary price change, a special pricing period due to brand/generic switches or even supply shortages. Outliers can alert the model that certain drugs/NDC's experience more volatility and price fluctuations or manipulations.
...
Now we move to a crucial step- I want to repeat the above exercise, for every NDC_Formatted in drug_ndc_detail_package. Earlier when I ran this code to get the old and new NADAC per units, it took 6 hours for 4,850 NDC's out of 93,415. It took so long because each NDC_Formatted could have any number of rows inside the NADAC dataset. However, since we are doing annual-level averages, my hope is that this works faster, rather than returning every single row for each NDC. 
...
[[[LONG RUN TIME]]] Each batch of 1000 NDC's takes about 15-18 minutes to run, so altogether about 27 hours for the 93,415 NDC's.
```{r}
library(httr)
library(jsonlite)
library(dplyr)
...
save_dir <- "C:/Users/Ron/OneDrive/Desktop/Personal - Important/Grad School Stuff/CUNY SPS/DATA 698 Spring 2025/NADAC_Batches"
dir.create(save_dir, showWarnings = FALSE, recursive = TRUE)

# Get yearly NADAC averages for a NDC
...
# Define file path
folder_path <- "C:/Users/Ron/OneDrive/Desktop/Personal - Important/Grad School Stuff/CUNY SPS/DATA 698 Spring 2025/NADAC_Batches"
...
output_dir <- "C:/Users/Ron/OneDrive/Desktop/Personal - Important/Grad School Stuff/CUNY SPS/DATA 698 Spring 2025/NADAC_Plots"
